...
const zones = [
  ['PC', 'Боевые станции для серьёзной игры', img(9), '01'],
  ['PS5', 'Большой экран. Большие эмоции.', img(2), '02'],
  ['VR', 'Погрузись в игру полностью', img(3), '03'],
  ['AUTO', 'Почувствуй каждый поворот', img(7), '04'],
  ['TENNIS', 'Игра вне экрана', img(2), '05'],
];
const prices = [
...
function Hardware() { return <section className="section hardware" id="hardware"><div className="hardware-visual" style={{backgroundImage:`url(${img(5)})`}}><span>SPEC / 01</span><strong>BUILT<br/><i>TO WIN.</i></strong></div><div className="hardware-info"><SectionHead eyebrow="03 / LOADOUT" title="Железо" copy="Каждая станция собрана так, чтобы ты думал только об игре."/><div className="spec-grid">{specs.map(([key,val,sub])=><div className="spec" key={key}><span>{key}</span><strong>{val}</strong><small>{sub}</small></div>)}</div></div></section> }
function Promo() { return <section className="section promo" id="promo"><div className="promo-image" style={{backgroundImage:`url(${img(7)})`}}/><div className="promo-copy"><span className="eyebrow">04 / SPECIAL OFFER</span><p className="promo-label">УТРЕННИЙ</p><h2>ТАРИФ</h2><div className="promo-time">10:00 <span>—</span> 16:00</div><div className="promo-price">400 <small>сом</small></div><p>Выгодное время для игр и побед.<br/>Полный фокус без суеты.</p><Button>Забронировать утро</Button></div></section> }
function Gallery() { const gallery=[9,2,3,5,7,14,15,16]; const [selected,setSelected]=useState(null); return <section className="section gallery" id="gallery"><SectionHead eyebrow="05 / INSIDE BUNKER" title="Внутри игры" copy="Не просто место. Атмосфера, в которую хочется возвращаться."/><div className="gallery-grid">{gallery.map((n,i)=><button key={n} className={'gallery-item gi-'+i} onClick={()=>setSelected(img(n))}><img src={img(n)} alt="BUNKER GAME ZONE" loading="lazy"/><span>VIEW ↗</span></button>)}</div>{selected&&<div className="lightbox" onClick={()=>setSelected(null)}><img src={selected} alt="BUNKER"/><button>×</button></div>}</section> }
