...
export default function CalculatorExperience({ Layout, Eyebrow, Button, Icon, repository, settingsRepository }) {
  ...
  useEffect(() => { settingsRepository?.get().then((settings) => { const next = { ...calculatorConfig, ...Object.fromEntries(Object.keys(calculatorConfig).filter((key) => settings[key] !== undefined).map((key) => [key, Number(settings[key]) || 0])) }; setConfig(next); setValues((current) => ({ ...next, bikePrice: current.bikePrice })); }); }, [settingsRepository]);
  ...
  const result = useMemo(() => calculateEstimate(values, config), [values, config]);
