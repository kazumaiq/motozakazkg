...
export default function MotorcycleDetailPage({ id, Layout, Eyebrow, Button, Icon, navigate, repository, ApplicationForm }) {
  ...
  return <Layout><main className="page detail-page"><div className="detail-breadcrumb"><a href="/catalog" onClick={(event) => { event.preventDefault(); navigate('/catalog'); }}>← Каталог</a><span>/</span><span>{bike.brand} / {bike.model}</span></div><section className="detail-product"><Gallery bike={bike}/><div className="detail-identity"><Eyebrow>{bike.brand} · {bike.type}</Eyebrow><span className="detail-demo-label">DEMO DATA · JAPAN SELECTION</span><h1>{bike.model}<small>{bike.year}</small></h1><p className="detail-description">{bike.description}</p><div className="detail-price-new"><span>ОРИЕНТИРОВОЧНАЯ ЦЕНА В КЫРГЫЗСТАНЕ</span><strong>{price(bike)}</strong><small>Цена на текущем этапе. Точная стоимость определяется после подтверждения параметров и доставки.</small></div><div className="detail-status"><i/> {labels[bike.status] || bike.status}</div><div className="detail-actions-new"><a className="button" href="#request">Запросить этот мотоцикл <Icon/></a><Button href="/calculator" variant="ghost">Рассчитать полную стоимость</Button></div></div></section><section className="detail-spec-section"><div className="detail-section-kicker">01 · TECHNICAL PROFILE</div><div className="detail-spec-grid-new"><div><span>Двигатель</span><strong>{bike.engineCc} см³</strong></div><div><span>Пробег</span><strong>{formatNumber(bike.mileage)} км</strong></div><div><span>Год</span><strong>{bike.year}</strong></div><div><span>Состояние</span><strong>{bike.condition}</strong></div><div><span>Тип</span><strong>{bike.type}</strong></div><div><span>Валюта</span><strong>{bike.currency}</strong></div></div></section><section className="detail-auction-section"><div><div className="detail-section-kicker">02 · JAPAN AUCTION</div><h2>Откуда<br/><span>начинается проверка.</span></h2><p>Демонстрационная информация о подборе. Источник и параметры лота подтверждаются перед участием в торгах.</p></div><div className="auction-facts-new"><div><span>Локация</span><strong>{bike.auctionLocation || 'Japan'}</strong></div><div><span>Статус</span><strong>{labels[bike.status] || bike.status}</strong></div><div><span>ID мотоцикла</span><strong>#{bike.id}</strong></div><div><span>Источник</span><strong>DEMO DATA</strong></div></div></section><section className="detail-sheet-section"><div className="detail-section-kicker">03 · AUCTION SHEET</div><div className="detail-sheet-layout"><div className="detail-sheet-visual"><img src="/assets/auction-sheet.svg" alt="Демонстрационный японский аукционный лист" /></div><div><h2>Проверяем<br/><span>до ставки.</span></h2><p>Аукционный лист является частью проверки. Сейчас используется демонстрационный документ без live-данных.</p><div className="sheet-facts-new"><div><span>Оценка</span><strong>—</strong></div><div><span>Пробег</span><strong>{formatNumber(bike.mileage)} км</strong></div><div><span>Состояние</span><strong>{bike.condition}</strong></div><div><span>Документы</span><strong>Уточняются</strong></div></div></div></div></section><section className="detail-trust-section"><div><div className="detail-section-kicker">04 · TRANSPARENCY</div><h2>Что мы<br/><span>проверяем.</span></h2></div><div className="trust-list-new">{[['01','Состояние','Изучаем описание и фотографии лота.'],['02','Пробег','Сверяем пробег с доступными документами.'],['03','Документы','Проверяем наличие информации до ставки.'],['04','Повреждения','Фиксируем замечания инспекции.']].map(([number, title, text]) => <div key={number}><span>{number}</span><div><strong>{title}</strong><p>{text}</p></div></div>)}</div></section><section className="detail-delivery-section"><div className="detail-section-kicker">05 · DELIVERY ROUTE</div><div className="delivery-route-new">{['JAPAN','AUCTION','PURCHASE','SHIPPING','BISHKEK'].map((item, index) => <div key={item}><span>0{index + 1}</span><strong>{item}</strong>{index < 4 && <i/>}</div>)}</div></section><section id="request" className="detail-request-section"><div><div className="detail-section-kicker">06 · REQUEST</div><h2>Этот мотоцикл<br/><span>может быть вашим.</span></h2><p>Заявка автоматически привязана к выбранной модели.</p></div><ApplicationForm bike={bike}/></section>{related.length > 0 && <section className="detail-related"><div className="detail-section-kicker">MORE FROM INVENTORY</div><div className="detail-related-head"><h2>Похожие<br/><span>мотоциклы.</span></h2><Button href="/catalog" variant="ghost">Весь каталог</Button></div><div className="inventory-grid">{related.map((item) => <CatalogCard key={item.id} bike={item} onOpen={(itemId) => navigate(`/catalog/${itemId}`)} />)}</div></section>}<div className="detail-mobile-cta"><a className="button" href="#request">Запросить мотоцикл <Icon/></a></div></main></Layout>;
}
